import { ReviewResult, FOOTER } from './types.js';
import { box, sectionHeader, severityColor, success, warn, error, info } from '../utils/logger.js';
...
export function formatTerminal(result: ReviewResult): string {
  ...
  if (result.security.findings.length === 0) {
    ...
    for (const finding of result.security.findings) {
      ...
      if (finding.line) locationParts.push(String(finding.line));
