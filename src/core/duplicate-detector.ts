import { PRData, ExistingItem, DuplicateResult, DuplicateMatch } from './types.js';
...
export function detectDuplicates(
  ...
  for (const item of existingItems) {
    // Skip self
    if (item.number === pr.number && item.type === 'pr') continue;

    const itemText = `${item.title} ${item.body}`;
    let score = combinedSimilarity(prText, itemText);
    const matchReasons: string[] = [`text similarity: ${score.toFixed(2)}`];
