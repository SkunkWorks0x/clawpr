import { PRData, VisionAlignment } from './types.js';
import { combinedSimilarity, tokenize } from './similarity.js';

function extractHeadingsAndFirstSentences(doc: string): string {
...
function buildPRIntentText(pr: PRData): string {
...
export function checkVisionAlignment(
  ...
  // Determine source
  const visionSource: 'VISION.md' | 'README.md' = visionDoc.includes('# Project Vision') || visionDoc.includes('## Mission')
    ? 'VISION.md'
    : 'README.md';
  ...
  const codeFiles = pr.files.filter(f =>
    /\.(ts|js|tsx|jsx|py|go|rs)$/.test(f.filename) &&
  ...
  const testFiles = pr.files.filter(f =>
    /\.test\.|\.spec\.|\/test\/|__tests__\//.test(f.filename)
  );
