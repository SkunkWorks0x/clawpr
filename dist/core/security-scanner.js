...
function redactContent(matched) {
    if (matched.length <= 8)
        return '****';
    return matched.slice(0, 4) + '*'.repeat(matched.length - 8) + matched.slice(-4);
}
export function scanSecurity(addedLines, files) {
    const findings = [];
    ...
    for (const addedLine of addedLines) {
        if (binaryFiles.has(addedLine.file))
            continue;
        for (const rule of SECURITY_RULES) {
            if (rule.fileFilter && !rule.fileFilter.test(addedLine.file))
                continue;
