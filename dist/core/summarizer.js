function computeScope(additions, deletions) {
...
function categorizeFile(filename) {
    ...
    if (/\.test\.|\.spec\.|\/test\/|__tests__\//.test(lower))
        return 'test';
    if (/(?:auth|security|permission|crypt)/.test(lower))
        return 'security';
    if (/\.md$|^docs\//.test(lower))
        return 'docs';
    if (/^package\.json$|^package-lock\.json$|^yarn\.lock$|^pnpm-lock\.yaml$/.test(lower))
        return 'deps';
    if (/\.ya?ml$|\.toml$|\.json$/.test(lower))
        return 'config';
    if (/\.ts$|\.js$|\.tsx$|\.jsx$|\.py$|\.go$|\.rs$/.test(lower))
        return 'feature';
    return 'feature';
}
export function summarize(pr) {
